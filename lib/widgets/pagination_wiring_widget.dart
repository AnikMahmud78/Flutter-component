        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Card.filled(
            ...
            child: const ListTile(
              ...
              subtitle: Text('Page and boundary controls synchronise through reactive state.'),
            ),
          ),
          ...
          Card.outlined(child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Active Page State: Page ${_pageIndex + 1}', style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 12),
              AnimatedContainer(
                duration: const Duration(milliseconds: 200),
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                decoration: BoxDecoration(
                  color: _showFlash ? colors.primaryContainer : colors.surfaceContainerHighest,
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text('${_pageIndex + 1} of $_pageCount Pages (State Synced)', style: const TextStyle(fontFamily: 'monospace')),
              ),
            ]),
          )),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: SizedBox(height: 48, child: FilledButton.icon(
              onPressed: _pageIndex > 0 ? () => _onPageChanged(_pageIndex - 1) : null,
              icon: const Icon(Icons.navigate_before_rounded), label: const Text('PREVIOUS PAGE'),
            ))),
            const SizedBox(width: 12),
            Expanded(child: SizedBox(height: 48, child: FilledButton.icon(
              onPressed: _pageIndex < _pageCount - 1 ? () => _onPageChanged(_pageIndex + 1) : null,
              icon: const Icon(Icons.navigate_next_rounded), label: const Text('NEXT PAGE'),
            ))),
          ]),
          ...
          const Card.outlined(child: ListTile(
            ...
            subtitle: Text('PASS • Pagination handlers trigger reactive re-rendering.'),
          )),
        ]),
      ),
    );
